...
7. statements like continue and break; can be both operator and operand so kept as operand otherwise, they would have been propagated upwards and merged into other operators
...
Example 
S : a B {$$ = processNodes("S",2,$1,createNode("B",$1,operand=true))};
...
int getNewId();
void writeTree(ASTNode* root, int id, std::string filename);
ASTNode* processNodes(std::string type, int n, ...);
...
start: file_input { if(DEBUG) printf("[PARSER]: parse success\n"); return 0; } {$$ = $1; root = $$; writeTree(root, getNewId(), "parse_tree.dot");}
     ;  

file_input: continued_file_input ENDMARKER {$$ = $1;}
          ;

continued_file_input: continued_file_input NEWLINE {$$ = $1;}
                    | continued_file_input stmt { if(DEBUG) printf("[PARSER]: NEXT STATEMENT TO COME UP\n\n"); } { 
...
stmt: simple_stmt   { if(DEBUG) printf("[PARSER]: simple_stmt selected \n"); } {
        $$ = $1;
}
    | compound_stmt { if(DEBUG) printf("[PARSER]: compound_stmt selected \n"); }  {
        $$ = $1;
    }
    ;
...
small_stmt: flow_stmt { if(DEBUG) printf("[PARSER]: flow_stmt selected \n"); } {
                $$ = $1;
        }
        | expr_stmt { if(DEBUG) printf("[PARSER]: expr_stmt selected \n"); } {
                $$ = $1;
        }
        | global_stmt {
                $$ = $1;
...
expr_stmt: testlist_star_expr annassign  { if(DEBUG) printf("[PARSER]: annotated assignment in expr_stmt\n"); } {
                $$ = processNodes("expr_stmt",2,
                        $1,
                        $2
                        );
        }
        | testlist_star_expr AUGASSIGN testlist { if(DEBUG) printf("[PARSER]: augmented assignment in expr_stmt\n"); } {
        ...
        | testlist_star_expr continued_expr_stmt { if(DEBUG) printf("[PARSER]: normal assignment in expr_stmt\n"); } {
                $$ = processNodes("expr_stmt",2,
                        $1,
                        $2
                        );
...
continued_expr_stmt: continued_expr_stmt EQUAL testlist_star_expr { if(DEBUG2) printf("[PARSER]: testlist added\n"); } {
...
testlist_star_expr: test continued_testlist COMMA {
        ...
        | test continued_testlist { if(DEBUG2) printf("[PARSER]: single test in testlist\n"); } {
...
flow_stmt: break_stmt { if(DEBUG) printf("[PARSER]: flow_stmt: break_stmt\n"); } {
                $$ = $1;
        }
        | continue_stmt { if(DEBUG) printf("[PARSER]: flow_stmt: continue_stmt\n"); } {
                $$ = $1;
        }
        | return_stmt { if(DEBUG) printf("[PARSER]: flow_stmt: return_stmt\n"); } {
                $$ = $1;
        }
        | raise_stmt { if(DEBUG) printf("[PARSER]: flow_stmt: raise_stmt\n"); } {
                $$ = $1;
        }
        ;
...
assert_stmt: ASSERT test COMMA test {
                $$ = processNodes("ssert_smt",4,
                        createNode("ASSERT",$1,false,true),
                        $2,
                        createNode("COMMA",$3,true,false),
                        $4
                        );
...
compound_stmt: if_stmt  { if(DEBUG) printf("[PARSER]: if_stmt selected \n"); } {
                $$ = $1;
        }
        | while_stmt { if(DEBUG) printf("[PARSER]: while_stmt selected \n"); } {
                $$ = $1;
        }
        | for_stmt { if(DEBUG) printf("[PARSER]: for_stmt selected \n"); } {
                $$ = $1;
        }
        | try_stmt { if(DEBUG) printf("[PARSER]: try_stmt selected \n"); } {
                $$ = $1;
        }
        | funcdef { if(DEBUG) printf("[PARSER]: func_def selected \n"); } {
                $$ = $1;
        }
        | classdef { if(DEBUG) printf("[PARSER]: class_def selected \n"); } {
                $$ = $1;
        }
        ;
...
trailers: trailers trailer {if(DEBUG2) printf("[PARSER]: trailer added\n"); } {
...
atom: OPENING_BRACKET testlist_comp CLOSING_BRACKET {
    ...
    | NAME {if(DEBUG2) printf("[PARSER]: atom 'NAME' used\n"); } {
        $$ = createNode("NAME",$1,false,true);
        }
    | NUMBER  {if(DEBUG2) printf("[PARSER]: atom 'NUMBER' used\n"); } {
        $$ = createNode("NUMBER",$1,false,true);
        }
    | strings {if(DEBUG2) printf("[PARSER]: atom 'STRING' used\n"); } {
        $$ = $1;
        }      
    | ELLIPSES_LITERAL {if(DEBUG2) printf("[PARSER]: atom 'ELLIPSES_LITERAL' used\n"); } {
...
continued_testlist: continued_testlist COMMA test  {
        ...
        | %empty  { if(DEBUG2) printf("[PARSER]: no further tests added\n"); $$ = NULL; 
        }      
        ;

trailer: OPENING_BRACKET arglist CLOSING_BRACKET { if(DEBUG2) printf("[PARSER]: arglist within brackets added \n"); } {
       ...
       | OPENING_BRACKET CLOSING_BRACKET {if(DEBUG2) printf("[PARSER]: empty brackets added \n"); } {
...
arglist: argument continued_argument COMMA {
        ...
        | argument continued_argument { if(DEBUG2) printf("[PARSER]: single argument added in arglist\n"); } {
...
argument: test comp_for {
        ...
        | test {if(DEBUG2) printf("[PARSER]: argument contains a single test\n"); } {
                $$ = $1;
        }
        ;
